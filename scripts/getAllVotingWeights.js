...
const Snapshot2 = require("../src/Weights")
const getReporters = require("./getReporters.js")
const getUsers = require("./getUsers.js")
const getTokenHolders = require("./getTokenHolders.js")
...
async function getAllVotes(node2) {
    ...
    let reporterList = await Reports.getAllWeights(newestBlock);
    let holderList = await Holders.getAllWeights(newestBlock);
    let userList = await Users.getAllWeights(newestBlock);
    console.log("blockNumber", newestBlock);
    console.log("voting stats", holderList, userList, reporterList);
}


getAllVotes(process.env.NODE_URL_OPTIMISM)
    .then(() => process.exit(0))
    .catch(error => {
        console.error(error);
        process.exit(1);
